...
We see there are many missing values.
```{r}
str(training.raw)
```
...
Remove time related data, since we wont use those.
```{r}
...
Assigned cleaned datasets to variables
```{r}
training.cleaned <- training.cleaned03
testing.cleaned <- testing.cleaned03
```

We split the training set further into test and train set
